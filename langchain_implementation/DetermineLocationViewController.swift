//
//  DetermineLocationViewController.swift
//  langchain_implementation
//
//  Created by Helen Lei on 8/6/23.
//
...
class DetermineLocationViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate {
...
//    var purpose = "celebrating the fourth of july"
    ...
    @objc func initialPrompt() {
        var initialInput = "Hi! Tell me what kind of place you want to travel to?"
        self.models.append(initialInput)
//        var chatResponse = ""
//        Task{
//            chatResponse = await tempBot.run(userInput: "output suggestion for a travel destination based on indicated preferences")
...
//        return cell
        ...
        if let cell = table.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as? ChatTableViewCell {
            cell.configure2(chatInput: models[indexPath.row], index: indexPath.row)
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        if let text = textField.text, !text.isEmpty{
            ...
            var chatResponse = ""
            Task{
                chatResponse = await tempBot.run(userInput: "output suggestion for a travel destination based on indicated preferences \(text)")
                self.models.append(chatResponse)
                DispatchQueue.main.async{
                    self.table.reloadData()
                }
